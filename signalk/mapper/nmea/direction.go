...
func (s HDT) GetMagneticHeading() (float64, error) {
	if s.True {
		return 0, fmt.Errorf("Heading is  true in sentence: %s", s)
	}
	return (unit.Angle(s.Heading) * unit.Degree).Radians(), nil
}

// GetMagneticHeading retrieves the magnetic heading from the sentence
func (s VHW) GetMagneticHeading() (float64, error) {
	return (unit.Angle(s.MagneticHeading) * unit.Degree).Radians(), nil
}
...
func (s RMC) GetMagneticVariation() (float64, error) {
	if s.Validity != goNMEA.ValidRMC {
		return 0, fmt.Errorf("The validity flag is set to %s in the sentence: %s", s.Validity, s)
	}
	return (unit.Angle(s.Variation) * unit.Degree).Radians(), nil
}
...
func (s VDMVDO) GetRateOfTurn() (float64, error) {
	if positionReport, ok := s.Packet.(goAIS.PositionReport); ok && positionReport.Valid {
		...
		if positionReport.RateOfTurn == rateOfTurnNotAvailable {
			return 0.0, errors.New("Rate of turn is not available")
		}
...
func (s RMC) GetTrueCourseOverGround() (float64, error) {
	...
	return (unit.Angle(s.Course) * unit.Degree).Radians(), nil
}
...
func (s VDMVDO) GetTrueCourseOverGround() (float64, error) {
	if positionReport, ok := s.Packet.(goAIS.PositionReport); ok && positionReport.Valid {
		if positionReport.Cog == 360 {
			return 0.0, errors.New("Course over ground is not available")
		}
...
func (s VTG) GetTrueCourseOverGround() (float64, error) {
	return (unit.Angle(s.TrueTrack) * unit.Degree).Radians(), nil
}
...
func (s VTG) GetmagneticCourseOverGround() (float64, error) {
	return (unit.Angle(s.MagneticTrack) * unit.Degree).Radians(), nil
}

// GetTrueHeading retrieves the true heading from the sentence
func (s HDT) GetTrueHeading() (float64, error) {
	if !s.True {
		return 0, fmt.Errorf("Heading is not true in sentence: %s", s)
	}
	return (unit.Angle(s.Heading) * unit.Degree).Radians(), nil
}
...
func (s THS) GetTrueHeading() (float64, error) {
	if s.Status != goNMEA.AutonomousTHS {
		return 0, fmt.Errorf("Heading status is not autonomous in sentence: %s", s)
	}
	return (unit.Angle(s.Heading) * unit.Degree).Radians(), nil
}
...
func (s VDMVDO) GetTrueHeading() (float64, error) {
	if positionReport, ok := s.Packet.(goAIS.PositionReport); ok && positionReport.Valid {
		if positionReport.TrueHeading == 511 {
			return 0.0, errors.New("True heading is not available")
		}
...
func (s VHW) GetTrueHeading() (float64, error) {
	return (unit.Angle(s.TrueHeading) * unit.Degree).Radians(), nil
}
