...
// ModbusFileCollector collects NMEA from a serial device file
type ModbusFileCollector struct {
	Path   string
	Config config.ModbusConfig
}
...
// NewModbusFileCollector creates an instance of a file collector
func NewModbusFileCollector(url *url.URL, cfg config.ModbusConfig) *ModbusFileCollector {
	return &ModbusFileCollector{
		Path:   url.String(),
		Config: cfg,
	}
}
...
func (c *ModbusFileCollector) Collect(socket mangos.Socket) {
...
func (c *ModbusFileCollector) receive(stream chan<- []byte) error {
	defer close(stream)

	logger.GetLogger().Info(
		"Start to collect Modbus data from a serial device",
		zap.String("Path", c.Path),
	)
	client, err := modbus.NewClient(&modbus.ClientConfiguration{
		URL:      c.Path,
		Speed:    uint(c.Config.Baudrate), // default
