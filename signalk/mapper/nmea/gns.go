...
func init() {
	goNMEA.MustRegisterParser("GNS", func(s goNMEA.BaseSentence) (goNMEA.Sentence, error) {
		...
		if p.Fields[3] != "" && p.Fields[4] != "" {
			result.Longitude = NewFloat64(WithValue(p.LatLong(3, 4, "longitude")))
		} else {
			result.Longitude = NewFloat64()
		}
		if p.Fields[2] != "" {
			result.SVs = NewInt64(WithValue(p.Int64(6, "SVs")))
		} else {
			result.SVs = NewInt64()
		}
		if p.Fields[7] != "" {
		...
		if p.Fields[9] != "" {
			result.Separation = NewFloat64(WithValue(p.Float64(9, "separation")))
		} else {
			result.Separation = NewFloat64()
		}
		if p.Fields[10] != "" {
			result.Age = NewFloat64(WithValue(p.Float64(10, "age")))
		} else {
			result.Age = NewFloat64()
		}
		if p.Fields[11] != "" {
			result.Station = NewInt64(WithValue(p.Int64(11, "station")))
		} else {
...
func (s GNS) GetPosition3D() (float64, float64, float64, error) {
	...
	for _, m := range s.Mode {
		if _, ok := validModi[m]; ok && !s.Longitude.isNil && !s.Latitude.isNil && !s.Altitude.isNil {
			return s.Latitude.value, s.Longitude.value, s.Altitude.value, nil
		}
	}
