...
func init() {
	goNMEA.RegisterParser("VWR", func(s goNMEA.BaseSentence) (goNMEA.Sentence, error) {
		...
		if p.Fields[2] != "" {
			result.WindSpeedInKnots = NewFloat64(WithValue(p.Float64(2, "WindSpeedInKnots")))
		} else {
			result.WindSpeedInKnots = NewFloat64()
		}
		if p.Fields[4] != "" {
			result.WindSpeedInMetersPerSecond = NewFloat64(WithValue(p.Float64(4, "WindSpeedInMetersPerSecond")))
		} else {
			result.WindSpeedInMetersPerSecond = NewFloat64()
		}
		if p.Fields[6] != "" {
			result.WindSpeedInKilometersPerHour = NewFloat64(WithValue(p.Float64(6, "WindSpeedInKilometersPerHour")))
		} else {
