...
$$
BEGIN
    INSERT INTO "static_data" ("context", "name") VALUES (NEW."context", NEW."value_text")
    ON CONFLICT("context") DO UPDATE SET "name" = NEW."value_text";
    RETURN NEW;
END;
$$
LANGUAGE 'plpgsql';
...
AFTER INSERT ON "key_value_data"
FOR EACH ROW
WHEN (NEW."path" = 'name')
EXECUTE PROCEDURE "update_name"();

CREATE OR REPLACE FUNCTION "update_mmsi"()
RETURNS TRIGGER AS
$$
BEGIN
    INSERT INTO "static_data" ("context", "mmsi") VALUES (NEW."context", NEW."value_text")
    ON CONFLICT("context") DO UPDATE SET "mmsi" = NEW."value_text";
    RETURN NEW;
END;
$$
LANGUAGE 'plpgsql';
