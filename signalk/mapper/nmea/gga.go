...
func (s GGA) GetNumberOfSatellites() (int64, error) {
	if !s.NumSatellites.isNil {
		return s.NumSatellites.value, nil
	}
...
func (s GGA) GetPosition3D() (float64, float64, float64, error) {
	if (s.FixQuality == goNMEA.GPS || s.FixQuality == goNMEA.DGPS) && !s.Longitude.isNil && !s.Latitude.isNil && !s.Altitude.isNil {
