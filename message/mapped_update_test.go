...
var _ = Describe("Update", func() {
	u1 := NewUpdate().WithSource(*NewSource().WithLabel("testsource").WithType("test").WithUuid(uuid.Must(uuid.NewV7()))).WithTimestamp(time.Now())
	u1.AddValue(NewValue().WithPath("testpath").WithValue(42))
	u2 := NewUpdate().WithSource(*NewSource().WithLabel("testsource").WithType("test").WithUuid(uuid.Must(uuid.NewV7()))).WithTimestamp(time.Now())
	u2.AddValue(NewValue().WithPath("testpath").WithValue(42))
	u2.AddValue(NewValue().WithPath("testpathpath").WithValue(false))
	u3 := NewUpdate().WithSource(*NewSource().WithLabel("testsource").WithType("test").WithUuid(uuid.Must(uuid.NewV7()))).WithTimestamp(time.Now())
	u3.AddValue(NewValue().WithPath("testpathpath").WithValue(false))
	u3.AddValue(NewValue().WithPath("testpath").WithValue(42))
	u4 := NewUpdate().WithSource(*NewSource().WithLabel("testsourcesource").WithType("test").WithUuid(uuid.Must(uuid.NewV7()))).WithTimestamp(time.Now())
